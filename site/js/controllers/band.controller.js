 ...
 function mybandController ($scope,$http,$stateParams) {
   ...
   $scope.getBandasById = function (id){
     ...
     $scope.getBandSongs = function (id){
         $http({
           method: 'GET',
           url: 'http://myconcertv2.cloudapp.net/PEDS/BandsService.svc/band/songs?idbanda='+id
           }).then(function successCallback(response) {
             var data = JSON.parse(response.data.getCancionesBandaResult);
   ...
   $scope.getBandSongs ( $scope.bandId);
